...
- si `Equipement de metier` est active, chaque emplacement de metier de chaque metier Midnight suivi est verifie : l'outil et ses deux accessoires doivent porter un objet au moins rare dont l'ilvl atteint le seuil, sinon le token `stuff xN` compte les emplacements a completer et son infobulle donne le motif de chacun (`vide`, `rarete sous rare`, `ilvl 206 < 232`)
...
- la meme option ajoute `RF non equipe` quand un outil Resourcefulness est possede mais que l'outil porte a une autre statistique : `outil RF` dit la possession, ce token dit le port. Comme `outil RF`, il est reserve aux metiers de craft
- pour l'alchimie, la meme option ajoute `MC sac` tant qu'aucun outil Multicrafting n'est present dans les sacs : YayaQueue equipe l'outil Multicrafting juste avant les crafts qui multicraftent, ce qui suppose un exemplaire en sac en plus de l'outil Resourcefulness porte
- `Acheter stuff YQ xN` ajoute a la queue YayaQueue le rang rare de ce qui manque : l'outil pour un emplacement d'outil fautif, autant d'accessoires distincts qu'il y a d'emplacements d'accessoire fautifs, et l'outil d'alchimie quand l'exemplaire Multicrafting manque en sac. Rien n'est re-ajoute si l'objet est deja demande dans la file
...
- deux limites assumees, rappelees dans l'infobulle du bouton : le rang de craft d'un objet ne change pas son itemID (seulement ses bonusId) et la statistique d'un outil est tiree au hasard sur l'exemplaire. YayaQueue achete l'annonce la moins chere de l'itemID demande, donc ni l'ilvl ni la statistique ne sont filtres : l'annonce doit etre verifiee avant l'achat. Le stock possede n'est pour la meme raison pas deduit du besoin
- `/ywt stuff` bascule le suivi, `/ywt stuff on` et `/ywt stuff off` le forcent ; la trace `gear[<skillLineID>]` de `/ywt log` donne le compte d'emplacements, les motifs, l'outil RF porte et l'outil MC en sac
