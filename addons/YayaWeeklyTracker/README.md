...
- `Acheter stuff YQ xN` ajoute a la queue YayaQueue le rang rare de ce qui manque : l'outil quand aucun exemplaire conforme n'est possede, autant d'accessoires distincts qu'il y a d'emplacements d'accessoire fautifs, et l'outil d'alchimie quand l'exemplaire Multicrafting manque en sac. Rien n'est re-ajoute si l'objet est deja demande dans la file
...
- `/ywt stuff` bascule le suivi, `/ywt stuff on` et `/ywt stuff off` le forcent ; la trace `gear[<skillLineID>]` de `/ywt log` donne le compte d'emplacements, les motifs, la possession d'un outil conforme (`tool=`), celle d'un outil Resourcefulness (`rfOwned=`) et celle d'un outil Multicrafting en sac (`mcBag=`)
