...
- l'option `Ouvrir automatiquement les conteneurs YWT` est desactivee par defaut ; si elle est activee, les conteneurs suivis sont ouverts automatiquement hors combat et hors interfaces sensibles ; apres trois echecs consecutifs sur un item jamais reussi, l'automatisation s'arrete et affiche un bouton securise ; les echecs sont conserves dans `YayaWeeklyTrackerAccountDB.autoOpenFailed[itemID]` et les ouvertures confirmees dans `YayaWeeklyTrackerAccountDB.autoOpenSuccessfulContainers[itemID]` ; un item deja reussi n'entre jamais dans `autoOpenFailed` : ses erreurs ulterieures sont retentees toutes les 90 s et affichent aussi le bouton securise apres trois echecs ; les deux tables sont mutuellement exclusives ; une reussite automatique ou manuelle reinitialise ses echecs et relance la file ; apres chaque recuperation depuis la boite aux lettres, elle attend 0,5 s apres le dernier evenement de courrier avant de rescanner les sacs
