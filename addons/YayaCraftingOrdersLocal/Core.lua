...
local defaults = {
	...
	ignoreConcentrationOrdersProfessions = {},
...
function ns.GetIgnoredConcentrationOrdersProfessions()
	...
	db.ignoreConcentrationOrdersProfessions = type(db.ignoreConcentrationOrdersProfessions) == "table"
			and db.ignoreConcentrationOrdersProfessions
		or {}
	return db.ignoreConcentrationOrdersProfessions
end

function ns.ShouldIgnoreConcentrationOrdersForProfession(skillLineID)
	skillLineID = tonumber(skillLineID)
	if not skillLineID then
		return false
	end

	return ns.GetIgnoredConcentrationOrdersProfessions()[skillLineID] == true
end

function ns.SetConfig(key, value)
	if not YayaCraftingOrdersDB then
		return
	end
	YayaCraftingOrdersDB[key] = value
...
function ns.InitializeDatabase()
	YayaCraftingOrdersDB = type(YayaCraftingOrdersDB) == "table" and YayaCraftingOrdersDB or {}
	MigrateDatabase(YayaCraftingOrdersDB)
	CopyDefaults(defaults, YayaCraftingOrdersDB)
end
...
function ns.InitializeCommonModules()
	if type(YayaCraftingOrdersDB) ~= "table" then
