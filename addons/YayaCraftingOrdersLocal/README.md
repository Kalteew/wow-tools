...
- à la première ouverture d'un métier, uniquement à proximité de la station : demander d'abord les commandes de patrons en headless avec le métier résolu par `GetChildProfessionInfo()` puis `GetBaseProfessionInfo()` ; l'onglet patrons reste un fallback borné de 3 secondes si la requête ou les données échouent
...
- un métier n'est marqué comme traité qu'après le callback de requête et le passage d'auto-queue réussis ; les logs `/ypo debug` corrèlent les IDs enfant/base/onglet, la requête, le fallback et les quantités YQ
