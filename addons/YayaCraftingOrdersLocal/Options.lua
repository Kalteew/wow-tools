...
local function GetProfessionChoice(skillLineID)
	...
	if type(info) == "table" then
		return {
			skillLineID = info.skillLineID or skillLineID,
			label = info.professionName or info.parentProfessionName or ("SkillLine " .. tostring(skillLineID)),
		}
	end

	local name = type(C_TradeSkillUI) == "table"
		and type(C_TradeSkillUI.GetTradeSkillLineInfoByID) == "function"
		and C_TradeSkillUI.GetTradeSkillLineInfoByID(skillLineID)
	...
	return {
		skillLineID = skillLineID,
		label = "SkillLine " .. tostring(skillLineID),
	}
end

local function GetProfessionChoices()
	local choices = {}
	local seen = {}

	local function AddChoice(skillLineID)
		local choice = GetProfessionChoice(skillLineID)
		if not choice or seen[choice.skillLineID] then
			return
		end

		seen[choice.skillLineID] = true
...
function Options:RebuildIgnoredConcentrationProfessionOptions()
	if not (self.panel and self.panel.ignoreConcentrationHeader and self.panel.footer) then
		return
	end

	local panel = self.panel
	panel.ignoredConcentrationProfessionButtons = panel.ignoredConcentrationProfessionButtons or {}

	local previous = panel.ignoreConcentrationHint
	local choices = GetProfessionChoices()
	for index, choice in ipairs(choices) do
		local checkbox = panel.ignoredConcentrationProfessionButtons[index]
		if not checkbox then
			checkbox = CreateCheckButton(
				panel,
				choice.label,
				"TOPLEFT",
				previous,
				...
				function(self)
					local selected = {}
					local current = ns.GetIgnoredConcentrationOrdersProfessions and ns.GetIgnoredConcentrationOrdersProfessions() or {}
					for skillLineID, enabled in pairs(current) do
						if enabled then
							selected[tonumber(skillLineID) or skillLineID] = true
						end
					end
					if self.skillLineID and self:GetChecked() then
						selected[self.skillLineID] = true
					elseif self.skillLineID then
						selected[self.skillLineID] = nil
					end
					ns.SetConfig("ignoreConcentrationOrdersProfessions", selected)
				end
			)
			checkbox:SetParent(panel.scrollChild)
			panel.ignoredConcentrationProfessionButtons[index] = checkbox
		end

		checkbox.skillLineID = choice.skillLineID
		local label = GetCheckButtonLabel(checkbox)
		if label then
	...
	for index = #choices + 1, #panel.ignoredConcentrationProfessionButtons do
		panel.ignoredConcentrationProfessionButtons[index]:Hide()
	end

	panel.footer:ClearAllPoints()
	panel.footer:SetPoint("TOPLEFT", previous, "BOTTOMLEFT", 0, -18)
end
...
function Options:BuildPanel()
	...
	panel.sourceAuctionator = CreateRadioButton(
		...
		function(self)
			...
			ns.SetConfig("pricingSource", "tsm")
		end
	)
	...
	panel.expensiveIngredientThreshold = CreatePercentSlider(
		panel,
		"YayaCraftingOrdersExpensiveIngredientThresholdSlider",
	...
	panel.ignoreConcentrationHeader = CreateLabel(
		panel,
		"GameFontNormal",
		L.OPTION_IGNORE_CONCENTRATION_PROFESSIONS_HEADER,
		"TOPLEFT",
	...
	panel.ignoreConcentrationHint = CreateLabel(
		panel,
		"GameFontHighlightSmall",
		L.OPTION_IGNORE_CONCENTRATION_PROFESSIONS_HINT,
		"TOPLEFT",
		panel.ignoreConcentrationHeader,
		"BOTTOMLEFT",
		0,
		-6,
		620
...
function Options:Refresh()
	...
	self.panel.sourceAuctionator:Hide()
	self.panel.sourceAuctioneer:Hide()
	self.panel.sourceAuctionator:SetChecked(false)
	self.panel.sourceAuctioneer:SetChecked(false)

	self.panel.noPricingInfo:ClearAllPoints()
	self.panel.noPricingInfo:SetPoint("TOPLEFT", self.panel.sourceHeader, "BOTTOMLEFT", 0, -8)
	self.panel.noPricingInfo:SetShown(not status.tsmDetected)

	self.panel.sourceFooter:ClearAllPoints()
	if not status.tsmDetected then
		self.panel.sourceFooter:Hide()
		lastPricingAnchor = self.panel.noPricingInfo
	else
		self.panel.sourceFooter:SetPoint("TOPLEFT", self.panel.sourceHeader, "BOTTOMLEFT", 0, -8)
		self.panel.sourceFooter:SetShown(true)
		if status.activeProviderKey == "tsm" then
			self.panel.sourceFooter:SetText(LF("OPTION_PRICING_FOOTER_TSM_ACTIVE", status.priceSource or "dbmarket"))
		elseif status.priceSourceError then
			self.panel.sourceFooter:SetText(
				LF("OPTION_PRICING_FOOTER_TSM_INVALID", status.priceSource or "dbmarket", status.priceSourceError)
			)
		else
			self.panel.sourceFooter:SetText(LF("OPTION_PRICING_FOOTER_TSM_READY", status.priceSource or "dbmarket"))
		end
	...
	self:RebuildIgnoredConcentrationProfessionOptions()
	local ignoredProfessions = ns.GetIgnoredConcentrationOrdersProfessions and ns.GetIgnoredConcentrationOrdersProfessions() or {}
	for _, checkbox in ipairs(self.panel.ignoredConcentrationProfessionButtons or {}) do
		if checkbox:IsShown() then
			checkbox:SetChecked(ignoredProfessions[checkbox.skillLineID] == true)
		end
	end
