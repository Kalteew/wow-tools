...
Case `Mode Cancel & Repost` dans le panneau `Paramètres` de l'onglet Reset. Une fois cochée, le même bouton d'action propose tour à tour d'acheter, d'annuler une enchère sous-cotée, ou de remettre une commodité en vente. Priorité : acheter, puis annuler, puis reposter.
...
- **annulation** : toutes tes enchères de commodités sont examinées. Une enchère est annulée si un autre vendeur est moins cher, ou s'il est au même prix — en Retail l'enchère postée après la tienne passe devant. `cancelRepost` annule aussi pour reposter plus haut quand la concurrence a disparu et que l'écart dépasse `cancelRepostThreshold`. `cancelUndercut = false` et `ignoreLowDuration` sont respectés ;
