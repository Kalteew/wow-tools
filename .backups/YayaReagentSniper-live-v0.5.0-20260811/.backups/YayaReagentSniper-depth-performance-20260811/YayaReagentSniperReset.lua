...
local function ReadDepth(active)
	...
	-- Les paliers suivants ne peuvent plus produire une opportunité si l'une
	-- de ces bornes est déjà dépassée. On évite alors de charger toute la profondeur.
	local budget = (tonumber(scan.db and scan.db.budgetGold) or 0) * 10000
	local dailyCapacity = (tonumber(active.candidate.soldPerDay) or 0)
		* (tonumber(scan.db and scan.db.marketShare) or 0) / 100
	local maxQuantity = dailyCapacity * (tonumber(scan.db and scan.db.maxDays) or 0)
	local maxTarget = (tonumber(active.candidate.referencePrice) or 0)
		* (tonumber(scan.db and scan.db.maxTargetPct) or 0) / 100
	local cumulativeQuantity = 0
	local cumulativeCost = 0
	for _, tier in ipairs(tiers) do
		cumulativeQuantity = cumulativeQuantity + tier.quantity
		cumulativeCost = cumulativeCost + tier.price * tier.quantity
		if cumulativeCost > budget or (maxQuantity > 0 and cumulativeQuantity > maxQuantity) then
			return true
		end
	end
	return tiers[#tiers].price >= maxTarget
end
...
local function HandleCommodityResults(itemID)
	...
	CancelTimer("timeout")
	local stopEarly = ReadDepth(active)
	if stopEarly then
