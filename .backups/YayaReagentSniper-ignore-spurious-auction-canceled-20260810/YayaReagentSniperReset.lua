...
function Reset:OnEvent(event, ...)
	...
	if event == "AUCTION_CANCELED" then
		...
		if scan.running then
			self:StopScan("Une de tes auctions a été annulée : relance le scan Reset.")
		else
			for _, result in ipairs(scan.opportunities) do
				InvalidateResetOpportunity(result, "Une de tes auctions a été annulée : résultat à rescanner.")
			end
			wipe(scan.depthCache)
			scan.selected = nil
			self:RefreshRows()
			SetStatus("Auction annulée : relance le scan Reset pour recalculer le marché.", true)
		end
		return
	end
