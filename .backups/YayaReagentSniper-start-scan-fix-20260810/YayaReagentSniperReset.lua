...
local function EvaluateDepth(candidate, tiers)
...
function Reset:RefreshSelected()
	...
	scan.frame.detailBody:SetText(string.format(
		"Score %d/100 • absorber %s palier(s) pour %s, puis viser %s/u. Profit net %s après 5%% de commission • %.1f j à %.0f%% de la demande TSM. Le prix Blizzard sera revalidé avant confirmation.",
...
local function FinishDeepCandidate(complete)
	...
	if complete then
		local result = EvaluateDepth(active.candidate, active.tiers or {})
...
function Reset:RebuildFromCache()
	...
	for _, cached in pairs(scan.depthCache) do
		if GetTime() - cached.scanAt <= CONSTANTS.MAX_SCAN_AGE then
			local result = EvaluateDepth(cached.candidate, cached.tiers)
			if result then
				scan.opportunities[#scan.opportunities + 1] = result
			end
		end
